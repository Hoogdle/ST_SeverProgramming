// 소스코드 출처:
// https://crazythink.github.io/2018/05/25/socketchat/
...
int main(int argc, char *argv[]) {
    ...
    // fd_set은 File Descriptor를 저장하는 구조체. 
    // https://hahaite.tistory.com/290 (fd 관련 글)
	pthread_t a_thread;
    ...
    // tcp_listen 함수는 1) 소켓 개설, 2) 서버주소 바인딩, 3) listen() 함수 호출을 모두 담고있는 함수
    // https://ddingz.tistory.com/254

	//스레드 생성
	pthread_create(&a_thread, NULL, thread_function, (void *)NULL);
    // 호출되는 프로세스의 새로운 스레드 시작,
    // thread_function의 함수를 시작
    // not 오류 => 0 리턴, 에러 발생 => error number 리턴

	while (1) {
        // FD, select, fd_set에 관한 글
		FD_ZERO(&read_fds); // fd_set으로 선언된 변수를 초기화, 위 선언에서 쓰레기 값이 있을 수 있음.
		FD_SET(listen_sock, &read_fds); // fd_socket 값을 
		for (i = 0; i < num_user; i++)
			FD_SET(clisock_list[i], &read_fds);

		maxfdp1 = getmax() + 1;	// maxfdp1 재 계산
		if (select(maxfdp1, &read_fds, NULL, NULL, NULL) < 0)
        // select 함수는 fd_set에 할당된 FD의 이벤트 감지, 어떤 이벤트가 발생했는지 알려줌.
			errquit("select fail");
